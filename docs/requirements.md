...
| FR-40 | Administrators can process a resolved catalog as a durable audited batch. | PostgreSQL checkpoints each item independently; content-hash matches skip unchanged content; create, update-draft, and new-revision preserve Published immutability; failed items can retry without duplicate revisions; only the content worker compiles or executes source; and publication requires an explicit list of approved Ready revision IDs. Internal responses, audit, and normal logs exclude private source and hidden testcase values. |
| FR-41 | Administrators can review and operate content batches in the browser. | Admin-only list and detail routes show Pending, Generating, Ready, Failed, Published, and Skipped counts; support slug/title search and status filters; poll active batches for a bounded period; display only safe diagnostics; retry one or many Failed items; and require confirmation plus explicit Ready revision selection before bulk publish. Regular users receive the forbidden UX, and private source or hidden testcase data never enters the browser contract. |
